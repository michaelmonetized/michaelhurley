...
- OpenRouter chat: use valid default `openrouter/free` (not `openrouter/openrouter/free`), resolve model via `openrouter(id)` like hustle-launch, and probe candidate ids once per process with `generateText`—logging failures and caching the first success.
