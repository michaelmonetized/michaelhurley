...
export default function HeroSlider({ className }: { className?: string }) {
  ...
  return (
    <section
      className={cn(
        "relative mx-auto flex w-full max-w-5xl flex-col items-center gap-10 px-6 py-24 md:py-32",
        className
      )}
    >
      <div className="relative h-[220px] w-full sm:h-[200px] md:h-[180px]">
        {slides.map((slide, index) => (
          ...
          >
            <h2 className="font-heading text-3xl font-black tracking-tight text-foreground md:text-4xl lg:text-5xl">
              {slide.headline}
            </h2>

            <p className="max-w-[42rem] text-sm text-muted-foreground md:text-base">
              {slide.subtitle}
            </p>
